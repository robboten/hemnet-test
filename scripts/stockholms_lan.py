...
parent_dir_path = "./data/stockholms_lan/"
...
with open("./configs/stockholms_lan_ids.yaml", "r") as stream:
...
print(f"\n{listings_count} listings in Stockholms län today! \n")

# area_info["listings_in_stockholms_lan"] = listings_count

# with open(f"{directory_path}stockholms_lan_{dateoftoday}.yml", "w") as outfile:
#     yaml.dump(area_info, outfile, default_flow_style=False, sort_keys=False)

# with open(f"{directory_path}stockholms_lan_{dateoftoday}.json", "w") as outfile:
#     outfile.write(json.dumps(area_info, sort_keys=False))

# with open(f"{directory_path}stockholms_lan_{dateoftoday}.txt", "w") as outfile:
#     outfile.write(json.dumps(area_info, sort_keys=False))
